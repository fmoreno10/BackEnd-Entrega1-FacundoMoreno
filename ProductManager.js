    ...
    */
    async addProduct(oProduct) {
        ...
        try {
            ...
            const productsInFile = await fs.readFile(this.#path, 'utf-8');
        } catch (err) {
            console.log(err);
            return id;
        }
...
let oNewProductID = await oProductManager.addProduct(oTestProduct);
...
console.log(await oProductManager.getProducts());
...
let oNewProductID2 = await oProductManager.addProduct(oTestProduct);
...
const oProductToFind = await oProductManager.getProductById(oNewProductID);
if (oProductToFind) {
...
const oProductToFind2 = await oProductManager.getProductById(nonExistentID);
if (oProductToFind2) {
...
let oNewProductID3 = await oProductManager.addProduct(oNewProduct);
...
await oProductManager.updateProduct(oNewProductID3,oModProduct);
console.log(await oProductManager.getProducts());

// 9. Eliminar el prodcuto con ID: oNewProductID3
await oProductManager.deleteProduct(oNewProductID3);
console.log(await oProductManager.getProducts());
